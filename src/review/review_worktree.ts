...
function deriveWorktreePath(cwd: string, reviewBranch: string): string {
  ...
  const commonBase =
    path.basename(commonDir) === ".git" ? `${path.basename(repoRoot)}.git` : path.basename(commonDir)
  const parentDir = path.basename(commonDir) === ".git" ? path.dirname(repoRoot) : path.dirname(commonDir)
...
export const reviewWorktree = tool({
