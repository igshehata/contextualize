...
import { CONTEXT_DIR } from "../context/git"
...
export function parseRawArguments(rawArguments: string): ParsedArguments {
  ...
  for (const token of tokens) {
    ...
    explicit[match[1] as string] = match[2] as string
  }
  ...
  if (positional.length >= 1) reviewBranchInput = positional[0] as string
  if (positional.length >= 2) targetBranch = positional[1] as string
  if (explicit.branch) reviewBranchInput = explicit.branch.trim() || null
  if (explicit.target) targetBranch = explicit.target.trim() || "develop"

  if (Object.prototype.hasOwnProperty.call(explicit, "features")) {
    requestedFeatures = (explicit.features as string)
      .split(",")
      .map((feature) => feature.trim())
      .filter(Boolean)
  }

  return { reviewBranchInput, targetBranch, requestedFeatures }
}
...
function walkFilesMatching(root: string, predicate: (filePath: string, name: string) => boolean): string[] {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && predicate(entryPath, entry.name)) results.push(entryPath)
    }
  }
...
function isFrontendRelevant(changedFiles: string[]): boolean {
  return changedFiles.some((file) => {
    ...
    return (
      [".tsx", ".jsx", ".css", ".scss", ".sass", ".less", ".mdx"].some((suffix) => lower.endsWith(suffix)) ||
...
function parsePorcelainPath(line: string): string | null {
  ...
  if (payload.includes(" -> ")) return payload.split(" -> ").pop() || null
  return payload
}
...
function fetchOriginRefs(cwd: string, refs: string[]): { ok: boolean; warning: string | null } {
  ...
  if (branches.length === 0) return { ok: true, warning: null }

  const result = tryGit(
    ["fetch", "--quiet", "--no-tags", "origin", ...branches.map((branch) => `+refs/heads/${branch}:refs/remotes/origin/${branch}`)],
    cwd,
  )
  if (result.ok) return { ok: true, warning: null }
...
function resolveReviewRef(cwd: string, activeBranch: string, requestedBranch: string | null) {
  if (!requestedBranch || requestedBranch === activeBranch) {
    return { reviewBranch: activeBranch, reviewRef: activeBranch }
  }
  ...
  if (!remoteSha.ok) return { reviewBranch: requestedBranch, reviewRef: requestedBranch }
  if (!localSha.ok) return { reviewBranch: requestedBranch, reviewRef: remoteRef }
  if (localSha.stdout === remoteSha.stdout) return { reviewBranch: requestedBranch, reviewRef: remoteRef }
...
export function prepareReviewContext(input: {
  ...
  try {
    ...
    if (inside !== "true") return errorResponse("Not inside a git repository or worktree.")

    const activeBranch = execGit(["branch", "--show-current"], cwd)
    if (!activeBranch) return errorResponse("Could not determine the active branch for this worktree.")
    ...
    if (!targetExists.ok) {
      return errorResponse(
        `Target ref ${targetRef} is not available locally${fetchWarning ? ` (${fetchWarning})` : ""}. Ensure the target branch exists on origin and your network is reachable, then rerun /review.`,
      )
    }
    ...
    const branchChangedFilesOutput = execGit(["diff", "--name-only", `${targetRef}...${reviewRef}`], cwd)
    ...
    const localChangedFiles = mode === "in_place" ? collectStatusFiles(statusOutput) : []
    ...
    // The project's factual context is maintained by contextualize under .agent/context
    // (core/ + features/), so the review loads the same facts the rest of the kit uses.
    const projectContextRoot = path.join(worktree, CONTEXT_DIR)
    ...
    if (fs.existsSync(agentsFile)) fallbackContextFiles.push(toRelative(worktree, agentsFile))
    if (fs.existsSync(readmeFile)) fallbackContextFiles.push(toRelative(worktree, readmeFile))
...
export const reviewContext = tool({
  ...
  async execute(args, context) {
    ...
    const result = prepareReviewContext({ cwd, worktree, rawArguments: args.rawArguments || "" })
